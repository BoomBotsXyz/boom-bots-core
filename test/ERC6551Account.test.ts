...
describe("ERC6551Account", function () {
  ...
  let erc6551Account1: ERC6551Account; // an account bound to a token
  let erc6551Account2: ERC6551Account; // an account bound to a token
  let erc6551Account3: ERC6551Account; // an account bound to a token
  ...
  describe("account execution", function () {
    it("non owner cannot execute", async function () {
      // no data
      await expect(erc6551Account1.connect(user2).execute(user2.address, 0, "0x", 0)).to.be.revertedWithCustomError(erc6551Account1, "ERC6551InvalidSigner");
      // erc20 transfer
      let calldata = erc20.interface.encodeFunctionData("transfer", [user2.address, 0]);
      await expect(erc6551Account1.connect(user2).execute(erc20.address, 0, calldata, 0)).to.be.revertedWithCustomError(erc6551Account1, "ERC6551InvalidSigner");
    });
