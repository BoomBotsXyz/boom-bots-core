...
import { BoomBots, BoomBotAccount, ERC2535Module, ERC6551AccountModule, MulticallModule, ERC20HolderModule, ERC721HolderModule, FallbackModule, RevertModule, Test1Module, Test2Module, Test3Module, ModulePack100, RingProtocolModuleA, BoomBotsFactory01, MockERC20, MockERC721, MockERC1155, DataStore, GasCollector } from "./../../typechain-types";
...
describe("modules/RingProtocolModuleA", function () {
  ...
  describe("setup", function () {
    ...
    it("can deploy modules", async function () {
      ...
      // RingProtocolModuleA
      ringProtocolModuleA = await deployContract(deployer, "RingProtocolModuleA", []) as RingProtocolModuleA;
  ...
  describe("install module", function () {
    ...
    it("can install and execute module", async function () {
      ...
      let delegatecalldata = accountProxy.interface.encodeFunctionData("executeRingProtocolModuleA", [ethAmount2])
      ...
      l1DataFeeAnalyzer.register("inst&exec RingProtocolModuleA", tx);
    ...
    it("can just execute module", async function () {
      ...
      let ethAmount2 = ethAmount1.mul(9).div(10)
      let delegatecalldata = accountProxy.interface.encodeFunctionData("executeRingProtocolModuleA", [ethAmount2])
      let tx = await accountProxy.connect(user1).executeRingProtocolModuleA(ethAmount2, {value: ethAmount1}); // and delegatecall
      //console.log('tx')
      //console.log(tx)
      let receipt = await tx.wait()
      //console.log('receipt')
      ...
      expect(amounts1[5]).gt(amounts0[5])
      expect(amounts1[6]).gt(amounts0[6])
      l1DataFeeAnalyzer.register("exec RingProtocolModuleA", tx);
      // cumulativeGasUsed: 1_000_520
    });
    it("cannot execute with zero eth", async function () {
      await expect(accountProxy.connect(user1).executeRingProtocolModuleA(0, {value: 100})).to.be.revertedWithCustomError(accountProxy, "AmountZero");
    });
    it("non owner cannot execute", async function () {
      await expect(accountProxy.connect(user2).executeRingProtocolModuleA(100, {value: 100})).to.be.revertedWithCustomError(accountProxy, "ERC6551InvalidSigner");
    });
  });
