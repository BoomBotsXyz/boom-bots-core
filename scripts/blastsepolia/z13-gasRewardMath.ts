...
// v0.1.1
const BOT_1_ADDRESS = "0x7CF5C2124dea60aC8dE90FA53CA611BA9d27628F" // BoomBotAccount v0.1.1
...
async function main() {
  ...
  gasBurner = await ethers.getContractAt(abi, GAS_BURNER_ADDRESS, boombotsdeployer) as MockGasBurner;
  ...
  await doMathAddr(BOOM_BOTS_NFT_ADDRESS, "BoomBotsNft", 1, 1);
...
async function claimMaxGas() {
  ...
  let tx = await gasBurner.connect(boombotsdeployer).claimMaxGas(gasBurner.address, {...networkSettings.overrides, gasLimit: 2_000_000})
  //console.log('tx')
  //console.log(tx)
...
async function claimAllGas() {
  ...
  let bal1 = await provider.getBalance(gasBurner.address)
  let tx = await gasBurner.connect(boombotsdeployer).claimAllGas(gasBurner.address, {...networkSettings.overrides, gasLimit: 2_000_000})
  //console.log('tx')
  //console.log(tx)
  await tx.wait(networkSettings.confirmations)
  let bal2 = await provider.getBalance(gasBurner.address)
...
async function doMath(contract:any, contractName:string, supportsQuoteClaimAllGas=0, supportsClaimGas=0) {
  ...
  try {
    ...
    var promises = [
      ...
      //contract.connect(boombotsdeployer).callStatic.quoteClaimMaxGas(overrides),
      //contract.connect(boombotsdeployer).callStatic.quoteClaimAllGas(overrides),
      //contract.connect(boombotsdeployer).callStatic.claimMaxGas(boombotsdeployer.address, overrides),
      //contract.connect(boombotsdeployer).callStatic.claimAllGas(boombotsdeployer.address, overrides),
      (() => { return 12345 })(),
      (() => { return 12345 })(),
      (() => { return 12345 })(),
      (() => { return 12345 })(),
    ...
    if(!!supportsQuoteClaimAllGas) {
      promises[3] = contract.connect(boombotsdeployer).callStatic.quoteClaimMaxGas(overrides)
      promises[4] = contract.connect(boombotsdeployer).callStatic.quoteClaimAllGas(overrides)
    }
    if(supportsClaimGas==1) {
      promises[5] = contract.connect(boombotsdeployer).callStatic.claimMaxGas(boombotsdeployer.address, overrides)
      promises[6] = contract.connect(boombotsdeployer).callStatic.claimAllGas(boombotsdeployer.address, overrides)
    }
    if(supportsClaimGas==2) {
      //promises[5] = contract.connect(boombotsdeployer).callStatic.claimMaxGasImpl(boombotsdeployer.address, overrides)
      //promises[6] = contract.connect(boombotsdeployer).callStatic.claimAllGasImpl(boombotsdeployer.address, overrides)
      promises[5] = contract.connect(boombotsdeployer).callStatic.zzz_implClaimMaxGas(boombotsdeployer.address, overrides)
      promises[6] = contract.connect(boombotsdeployer).callStatic.zzz_implClaimAllGas(boombotsdeployer.address, overrides)
    }
    if(supportsClaimGas==3) {
      ...
      promises[5] = contract.connect(boombotsdeployer).callStatic.execute(BLAST_ADDRESS, 0, calldata5, 0, overrides)
      promises[6] = contract.connect(boombotsdeployer).callStatic.execute(BLAST_ADDRESS, 0, calldata6, 0, overrides)
    }
...
async function doMathAddr(address:string, contractName:string, supportsQuoteClaimAllGas=0, supportsClaimGas=0) {
  var contract = await ethers.getContractAt(abi, address, boombotsdeployer) as MockGasBurner;
