...
//const GAS_BURNER_ADDRESS              = "0x97B1cb36FacAB8DDB04B253A16d8F8bf4Cd77904"; // 2
//const GAS_BURNER_ADDRESS              = "0xd2CD92ef6Fa8B8330ecC6bfc91616387fc7Ee39b"; // 1
...
async function main() {
  ...
  //gasBurner = await ethers.getContractAt("MockGasBurner", GAS_BURNER_ADDRESS, boombotsdeployer) as MockGasBurner;
...
async function deployGasBurner() {
  if(await isDeployed(GAS_BURNER_ADDRESS)) {
    ...
    gasBurner = await deployContractUsingContractFactory(boombotsdeployer, "MockGasBurner", args, toBytes32(0), undefined, {...networkSettings.overrides, gasLimit: 6_000_000}, networkSettings.confirmations) as MockGasBurner;
...
async function configureGasBurner() {
  ...
  //let txdatas = []
  let tx = await gasBurner.connect(boombotsdeployer).multicall(txdatas, {...networkSettings.overrides, gasLimit: 500_000})
  console.log('tx')
  console.log(tx)
...
async function burnGas() {
  ...
  let tx = await gasBurner.connect(boombotsdeployer).burnGas(1000, {...networkSettings.overrides, gasLimit: 2_000_000})
  console.log('tx')
  console.log(tx)
  await tx.wait(networkSettings.confirmations)
...
async function doMath() {
  ...
  var res = await Promise.all([
    ...
    gasBurner.connect(boombotsdeployer).callStatic.claimMaxGas(boombotsdeployer.address, overrides),
    gasBurner.connect(boombotsdeployer).callStatic.claimAllGas(boombotsdeployer.address, overrides),
    gasBurner.connect(boombotsdeployer).callStatic.quoteClaimMaxGas(overrides),
    gasBurner.connect(boombotsdeployer).callStatic.quoteClaimAllGas(overrides),
  ])
