...
import { BoomBots, BoomBotAccount, ModulePack100, BoomBotsFactory01, BoomBotsFactory02, DataStore, RingProtocolModuleB, IBlast, BalanceFetcher, MockERC20 } from "../../typechain-types";
...
let botID35 = 35
let botAddress35 = "0xA6E8a482c21245A6d574ce05b23b3Dce4292E79F"
let implAddress35 = "0x8EA19CA269A3F3A7563F7A098C9C3dC46f4A2448"
let botowner = blasttestnetuser3
...
async function main() {
  ...
  accountProxy6 = await ethers.getContractAt(abi, botAddress35, botowner);
  iblast = await ethers.getContractAt("IBlast", BLAST_ADDRESS, botowner) as IBlast;

  ringProtocolModuleB = await ethers.getContractAt("RingProtocolModuleB", RING_PROTOCOL_MODULE_B_ADDRESS, botowner) as RingProtocolModuleB;
...
async function fetchBotBalances() {
  console.log("Fetching bot 35 balances")

  let balances = await balanceFetcher.callStatic.fetchBalances(botAddress35, TOKEN_LIST, {...networkSettings.overrides, gasLimit: 30_000_000})
  ...
  var hardcall = false
  if(hardcall) {
    let tx = await balanceFetcher.fetchBalances(botAddress35, TOKEN_LIST)
    console.log('tx')
    console.log(tx)
    await tx.wait(networkSettings.confirmations)
  }

  console.log("Fetched bot 35 balances")
}

// install and execute ring protocol module a
async function useBot35_1() {
  console.log("Using bot 35_1")
  ...
  */
  ...
  let tx = await accountProxy6.connect(botowner).diamondCut(cut, RING_PROTOCOL_MODULE_B_ADDRESS, delegatecalldata, {...networkSettings.overrides, value: ethAmount1, gasLimit: 2_000_000}); // and delegatecall
  await tx.wait(networkSettings.confirmations)
  console.log("Used bot 35_1")
}

// execute ring protocol module b
async function useBot35_2() {
  console.log("Using bot 35_2")
  let ethAmount1 = WeiPerEther.mul(2).div(1000)
  let ethAmount2 = ethAmount1.mul(9).div(10)
  let tx = await accountProxy6.connect(botowner).executeRingProtocolModuleB(ethAmount2, {...networkSettings.overrides, value: ethAmount1, gasLimit: 2_000_000});
  await tx.wait(networkSettings.confirmations)
  console.log("Used bot 35_2")
}

// claim max gas rewards
async function useBot35_6() {
  console.log("Using bot 35_6")
  let calldata = iblast.interface.encodeFunctionData("claimMaxGas", [botAddress35, botAddress35])
  let tx = await accountProxy6.connect(botowner).execute(BLAST_ADDRESS, 0, calldata, 0, {...networkSettings.overrides, gasLimit:1_000_000})
  //console.log('tx')
  //console.log(tx)
  await tx.wait(networkSettings.confirmations)
  console.log("Used bot 35_6")
}

// claim all gas rewards
async function useBot35_7() {
  console.log("Using bot 35_7")
  let calldata = iblast.interface.encodeFunctionData("claimAllGas", [botAddress35, botAddress35])
  let tx = await accountProxy6.connect(botowner).execute(BLAST_ADDRESS, 0, calldata, 0, {...networkSettings.overrides, gasLimit:1_000_000})
  //console.log('tx')
  //console.log(tx)
  await tx.wait(networkSettings.confirmations)
